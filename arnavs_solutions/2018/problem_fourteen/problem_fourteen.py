with open("Prob14.in.txt") as input_file:
    ...
    data = [([int(l) for l in lines[1].split()], "".join(c for c in data if c in "ABCDEFGHIJKLMNOPQRSTUVWXYZ-").replace("-", " ")),
     ([int(l) for l in lines[3].split()], "".join(c for c in data if c in "abcdefghijklmnopqrstuvwxyz=\n").replace("=", " "))]
